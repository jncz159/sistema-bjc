...
export default function SistemaBJCMasterFinal() {
  ...
  const getFechaPeru = (dateInput = new Date()) => {
    ...
    return partes.find(p => p.type === 'year').value + '-' +
  ...
  useEffect(() => {
    ...
    cargarTodo();
    const canal = supabase.channel('bj-realtime-v6')
  ...
  const finalizarVentaLote = async (estado = 'Entregado', conWA = false) => {
    ...
    if (!error) {
      ...
      if (conWA && telefono) {
        ...
        m += `%0A%0A*TOTAL: S/ ${(totalCarrito - descuento).toFixed(2)}*%0A¡Gracias! 😊`;
  ...
  const exportarExcel = () => {
    ...
    ventasFiltradas.forEach(v => {
      csv += `${getFechaPeru(v.created_at)},${v.cliente_nombre},${v.localidad},${v.estado_pedido},${productos.find(p=>p.id===v.producto_id)?.nombre},${v.cantidad},${(v.precio_venta_unitario * v.cantidad).toFixed(2)}\n`;
    });
    const link = document.createElement("a");
    link.href = encodeURI(csv);
    link.download = `Respaldo_BJ_${fechaConsulta}.csv`;
    link.click();
  };
        ...
        {vista === 'ventas' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
            ...
            <div style={{ ...glassCard, border: `2px solid #FCA5D4` }}>
              ...
              <input placeholder="🔍 Buscar producto para agregar..." value={busqueda} onChange={e => setBusqueda(e.target.value)} style={{ ...bjInput, border: `2px solid #FCC2E2`, marginBottom: '20px' }} />
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '15px', maxHeight: '450px', overflowY: 'auto' }}>
                {productos.filter(p => p.nombre.toLowerCase().includes(busqueda.toLowerCase())).map(p => (
                  <div key={p.id} style={{ border: '1px solid #FFF1F2', padding: '15px', borderRadius: '18px', backgroundColor: '#fff', position: 'relative' }}>
                    ...
                    <select value={coloresElegidos[p.id]} onChange={e => setColoresElegidos({...coloresElegidos, [p.id]: e.target.value})} style={{ ...bjInput, padding: '5px', fontSize: '11px', marginBottom: '8px', marginTop: '5px' }}>
                      {p.colores?.split(',').map(c => <option key={c} value={c.trim()}>{c.trim()}</option>)}
                    </select>
                    
                    {/* RESTAURADO: SELECTOR DE CANTIDAD */}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                {ventasAgrupadas.map(g => (
                  <div key={g.id_principal} style={{ padding: '18px', backgroundColor: '#FFF5F7', borderRadius: '18px', border: `1px solid ${g.items.some(i=>i.estado_pedido==='En Almacén') ? FUCSIA_PRINCIPAL : '#FCC2E2'}` }}>
                    ...
                    {g.items.map(v => (
                      <div key={v.id} style={{ backgroundColor: '#fff', padding: '12px', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
                        {idVentaEditando === v.id ? (
                          ...
                          <>
                            ...
                            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                              ...
                              {v.estado_pedido === 'En Almacén' && <button onClick={async () => { if(confirm("Confirmar entrega?")) await supabase.from('ventas').update({estado_pedido:'Entregado'}).eq('id', v.id); }} style={{ backgroundColor: '#1E1B1C', color: '#fff', border: 'none', padding: '5px 10px', borderRadius: '6px', fontSize: '10px', fontWeight: 'bold' }}>ENTREGAR</button>}
                    ...
                    <div style={{ textAlign: 'right', fontWeight: '900', fontSize: '15px', marginTop: '10px' }}>Total Pedido: S/ {g.total.toFixed(2)}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '25px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
                ...
                {/* RESTAURADO: FORMULARIO AGREGAR PRODUCTO */}
                <div style={{ ...glassCard, border: `2px solid ${FUCSIA_PRINCIPAL}` }}>
                  ...
                  <form onSubmit={agregarProductoNuevo} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <input placeholder="Colores (separados por comas)" value={formProd.colores} onChange={e => setFormProd({...formProd, colores: e.target.value})} style={bjInput} />
