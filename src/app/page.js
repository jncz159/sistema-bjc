...
/**
 * ============================================================================
 * SISTEMA: BUNKER BJ - MASTER CONTROL (v1.2.8 TOTAL AUDIT)
 * PROPIETARIO: Jean - B J Importaciones Chiclayo
 * COMPONENTES: 
 * - Auditoría de Caja Negra (Snapshots de dinero real)
 * - Excel Forense (Flujo de caja histórico)
 * - Autocompletado de Clientes Frecuentes
 * - Gestión de Logística Quirúrgica (Edición ítem por ítem)
 * ============================================================================
 */
...
export default function SistemaBJCMasterFinal() {
  ...
  // [BLOQUE A: ESTADOS GLOBALES - SIN SIMPLIFICAR]
  ...
  // [BLOQUE B: ESTADOS DE CONTROL DE VENTA]
  ...
  // [BLOQUE C: ESTADOS DE EDICIÓN Y FORMULARIOS]
  ...
  // [BLOQUE E: CARGA Y SINCRONIZACIÓN]
  ...
  const cargarTodoDesdeNube = async () => {
    ...
    } catch (e) { console.error("Error BJ Cloud:", e); } finally { setCargando(false); }
  };

  // [BLOQUE F: INTELIGENCIA FINANCIERA - CÁLCULOS MAESTROS]
  
  // 1. Balance Elite: Caja Real Auditada
  const balanceEliteBJ = useMemo(() => {
    ...
    // Suma de dinero REAL por ventas pagadas
    ...
    // Flujo administrativo (Solo lo que registras manualmente)
    const ingresosAdm = finanzas.filter(f => ['Ingreso Adicional', 'Inversión Inicial'].includes(f.tipo)).reduce((acc, f) => acc + Number(f.monto), 0);
    const gastosAdm = finanzas.filter(f => !['Ingreso Adicional', 'Inversión Inicial'].includes(f.tipo)).reduce((acc, f) => acc + Number(f.monto), 0);

    const cajaFisicaBJ = (totalVentasPagado + ingresosAdm) - gastosAdm;
  ...
  // 3. Logística: Agrupación detallada
  ...
  // 4. Historial Grupal (CON PRECIOS POR ÍTEM)
  const historialVentasDiaBJ = useMemo(() => {
    ...
    filt.forEach(v => {
        ...
        // Incluimos el precio unitario en cada ítem del historial
        groups[hId].items.push({ ...v, precio_historico: v.precio_venta_unitario }); 
  ...
  // [BLOQUE G: MANEJADORES - SIN SIMPLIFICAR]
  ...
  // 2. Ejecutar Venta con Snapshot de Caja Negra
  ...
  // 3. EXCEL FORENSE: Transparencia total de dinero
  const handleExportarExcelCajaFull = () => {
    let csv = "\uFEFF"; 
    csv += "CLIENTE,HORA,PRODUCTO,CANT,VALOR VENTA UNIT,SUBTOTAL,CAJA ACUM. DIA,SNAPSHOT CAJA GLOBAL\n";
    
    let cajaDiaAcumulada = 0;
    
    historialVentasDiaBJ.forEach(grupo => {
        grupo.items.forEach(it => {
            const prodNombre = productos.find(p => p.id === it.producto_id)?.nombre || "Modelo";
            const sub = it.cantidad * it.precio_historico;
            cajaDiaAcumulada += sub;
            
            // Localizamos la auditoría exacta para ver qué dinero había en ese momento
            const audit = auditoriaLogs.find(l => l.cliente === grupo.cliente_nombre && getHoraPeru(l.created_at) === grupo.hora);
            const globalSnapshot = audit ? audit.caja_despues : balanceEliteBJ.cG;

            csv += `${grupo.cliente_nombre},${grupo.hora},${prodNombre},${it.cantidad},${it.precio_historico},${sub},${cajaDiaAcumulada},${globalSnapshot}\n`;
        });
    });
    ...
    link.setAttribute("download", `BUNKER_AUDIT_${fechaConsulta}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  // 4. Gestión de Deudas (Sin simplificar)
  const handleCobrarDeudaBJ = async (grupo, montoFinal) => {
    const snapshot = balanceEliteBJ.cG;
    if(confirm(`Cobrar S/ ${montoFinal}?`)) {
        ...
        if(Number(montoFinal) > 0) {
            await supabase.from('auditoria_bj').insert([{ 
                ...
                monto_operacion: Number(montoFinal), caja_antes: snapshot, caja_despues: snapshot + Number(montoFinal) 
            }]);
        }
        cargarTodoDesdeNube();
    }
  ...
  // 5. Edición Quirúrgica en Logística (Sincronizada con Stock)
  ...
  if (cargando) return <div style={{ height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', backgroundColor:'#FFF5F7', color:FUCSIA_PRINCIPAL, fontWeight:'900' }}>BUNKER BJ v1.2.8 - AUDITANDO TOTAL... 🚀</div>;
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#FFF5F7', color: OSCURO_BJ, fontFamily: 'system-ui, sans-serif' }}>
      <header style={{ backgroundColor: '#ffffff', padding: '15px 5%', position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: `0 4px 15px rgba(0,0,0,0.06)` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div><h1 style={{ margin: 0, fontSize: '1.1rem', fontWeight: '900', color: FUCSIA_PRINCIPAL }}>BJ IMPORTACIONES</h1><small style={{ color: '#64748B', fontWeight: '900', fontSize: '9px' }}>v1.2.8 TOTAL AUDIT</small></div>
        </div>
