...
export default function SistemaBJCMasterFinal() {
  ...
  // [BLOQUE A: ESTADOS GLOBALES DE LA APLICACIÓN]
  ...
  // [BLOQUE D: ESTILOS Y COLORES CORPORATIVOS]
  ...
  const handleEjecutarVentaBJ = async (estado, abonoInicial = 0) => {
    ...
    const lista = carrito.map(i => {
        const gan = (Number(i.precio_venta) - Number(i.precio_compra)) * i.cantidad;
        return { 
            ...
            precio_costo_unitario: i.precio_compra, ganancia_total: gan, 
  ...
  const handleRegistrarFinanzaBJ = async (e) => {
    ...
    if (!error) {
        ...
        cargarTodoDesdeNube();
        alert("📖 Registro Administrativo Guardado.");
    }
  };
  ...
  const handleAnularCreditoBJ = async (grupo) => {
    if (confirm(`¿Anular crédito de ${grupo.cliente}?`)) {
      ...
      cargarTodoDesdeNube();
      alert("🗑️ Crédito eliminado.");
    }
  };

  const handleAnularVentaBJ = async (v) => {
    if (confirm("¿Anular esta venta y devolver stock?")) {
        const pO = productos.find(p => p.id === v.producto_id);
        if (pO) await supabase.from('productos').update({ stock: pO.stock + v.cantidad }).eq('id', pO.id);
        await supabase.from('ventas').delete().eq('id', v.id);
        cargarTodoDesdeNube();
