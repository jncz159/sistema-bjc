...
/**
 * ============================================================================
 * SISTEMA: BUNKER BJ - MASTER CONTROL (v1.4.5 TOTAL STABILITY)
 * PROPIETARIO: Jean - B J Importaciones Chiclayo
 * AUDITORÍA TÉCNICA:
 * - Restauración completa de handlers (Sin simplificar).
 * - Bóveda de Utilidad Pura: (Suma de utilidades de ventas no anuladas).
 * - Caja Física Actual: (Dinero real = Ventas Cobradas + Inversiones - Gastos).
 * - Formulario de Gestión conectado a Auditoría y Finanzas.
 * - Soporte para Tickets de WhatsApp, Stock en Logística y Auditoría Forense.
 * ============================================================================
 */
...
export default function SistemaBJCMasterFinal() {
  ...
  // [BLOQUE A: ESTADOS GLOBALES DE PERSISTENCIA]
  ...
  // [BLOQUE B: ESTADOS DE CONTROL OPERATIVO]
  ...
  // [BLOQUE F: INTELIGENCIA FINANCIERA (MEMO)]
  ...
  const balanceEliteBJ = useMemo(() => {
    ...
    // CAJA FÍSICA: Dinero que realmente entró - Dinero que realmente salió.
    ...
    const ingresosAdm = finanzas
        .filter(f => ['Ingreso Adicional', 'Inversión Inicial'].includes(f.tipo))
        .reduce((acc, f) => acc + Number(f.monto), 0);

    const gastosAdm = finanzas
        .filter(f => !['Ingreso Adicional', 'Inversión Inicial'].includes(f.tipo))
        .reduce((acc, f) => acc + Number(f.monto), 0);

    const cajaFisicaReal = (ingresosVentasCash + ingresosAdm) - gastosAdm;

    // BÓVEDA PARA RETIRO: Utilidad acumulada por ventas.
    const utilidadTotal = ventas
  ...
  // [BLOQUE G: MANEJADORES DE ACCIÓN]
  ...
  const handleAnularVentaBJ = async (v) => {
    if (confirm("¿Anular venta y devolver stock/dinero?")) {
  ...
  const handleExportarExcelCajaFull = () => {
    let csv = "\uFEFF"; csv += "CLIENTE,HORA,PRODUCTO,CANT,VALOR VENTA UNIT,SUBTOTAL,CAJA DIA,SNAPSHOT CAJA GLOBAL\n";
    let acumDia = 0;
    ...
    link.setAttribute("download", `BJ_AUDIT_${fechaConsulta}.csv`);
    document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };

  if (!hasMounted) return null;
  if (cargando) return <div style={{ height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', backgroundColor:'#FFF5F7', color:FUCSIA_PRINCIPAL, fontWeight:'900' }}>BUNKER BJ v1.4.5 - AUDITANDO TOTAL... 🚀</div>;
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#FFF5F7', color: OSCURO_BJ, fontFamily: 'system-ui, sans-serif' }}>
      <header style={{ backgroundColor: '#ffffff', padding: '15px 5%', position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: `0 4px 15px rgba(0,0,0,0.06)` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div><h1 style={{ margin: 0, fontSize: '1.1rem', fontWeight: '900', color: FUCSIA_PRINCIPAL }}>BJ IMPORTACIONES</h1><small style={{ color: '#64748B', fontWeight: '900', fontSize: '9px' }}>v1.4.5 MASTER</small></div>
        </div>
      ...
      <main style={{ maxWidth: '1400px', margin: '0 auto', padding: '30px 20px' }}>
        ...
        {vista === 'contabilidad' && <GestionSection {...{ 
            ...
            formFinanzas, setFormFinanzas, handleRegistrarFinanzaBJ, 
