...
export default function SistemaBJCMasterFinal() {
  ...
  // ============================================================
  // [BLOQUE 1] HELPERS (UTILIDADES DE TIEMPO Y SEGURIDAD)
  // ============================================================

  // Garantiza que siempre tengamos una fecha válida (Formato YYYY-MM-DD)
  ...
  // Formato de hora seguro (12H AM/PM)
  ...
  // Limpiador de montos (Previene errores por comas o nulos)
  ...
  // Etiquetas para productos nuevos o recientes
  ...
  // Estados para ediciones y correcciones
  ...
  // Estados para el panel de Gestión
  ...
  // ============================================================
  // [BLOQUE 3] NÚCLEO DE DATOS (REALTIME SUPABASE)
  // ============================================================
  ...
  useEffect(() => {
    ...
    const canalV = supabase.channel('v46-v').on('postgres_changes',{event:'*',schema:'public',table:'ventas'},()=>cargarTodoDesdeNube()).subscribe();
    const canalP = supabase.channel('v46-p').on('postgres_changes',{event:'*',schema:'public',table:'productos'},()=>cargarTodoDesdeNube()).subscribe();
    const canalF = supabase.channel('v46-f').on('postgres_changes',{event:'*',schema:'public',table:'finanzas'},()=>cargarTodoDesdeNube()).subscribe();
  ...
  const cargarTodoDesdeNube = async () => {
    ...
    } catch (err) { console.error("Error en sincronización Supabase:", err); }
  };

  // ============================================================
  // [BLOQUE 4] LÓGICA DE NEGOCIO (MEMOS CON ESCUDO ANTI-CRASH)
  // ============================================================

  // 1. LOGÍSTICA: Clasificación por entrega y cobranza
  ...
  // 2. HISTORIAL: Agrupa ventas del día (CORREGIDO ERROR DE VARIABLE)
  const historialVentasMaster = useMemo(() => {
    ...
    try {
        ...
        filtradas.forEach(v => {
            ...
            const llaveUnica = `${v.cliente_nombre || 'SN'}-${v.localidad || 'SZ'}-${hK}`; 
            if (!agrupadas[llaveUnica]) {
                agrupadas[llaveUnica] = { id_grupo: llaveUnica, cliente_nombre: v.cliente_nombre, localidad: v.localidad, telefono: v.telefono, hora: getHoraPeru(v.created_at), total: 0, items: [] };
            }
            agrupadas[llaveUnica].items.push(v);
            agrupadas[llaveUnica].total += (Number(v.precio_venta_unitario ?? 0) * Number(v.cantidad ?? 0));
        });
        return Object.values(agrupadas).reverse();
    } catch (e) { return []; }
  }, [ventas, fechaConsulta]);

  // 3. BALANCE FINANCIERO CRÍTICO: Caja Global y Bóveda
  const balanceFinancieroMaster = useMemo(() => {
    try {
        ...
        // Dinero real de ventas ya pagadas
        const vHoyCash = vL.filter(v => v && getFechaPeru(v.created_at) === hoyStr && v.estado_pedido !== 'Pendiente de Pago');
        
        // BÓVEDA DE GANANCIAS: (Ganancia real acumulada - Retiros desde bolsa Ganancias)
        const totalGananciaBrutaVentas = vL.reduce((acc, v) => acc + (Number(v?.ganancia_total) || 0), 0);
        const totalGastosDeGanancias = fL.filter(f => f && f.origen === 'Ganancias').reduce((acc, f) => acc + (Number(f.monto) || 0), 0);

        // CAJA GLOBAL (Capital + Ventas - Egresos)
        const totalEntradasVentas = vL.filter(v => v && v.estado_pedido !== 'Pendiente de Pago').reduce((acc, v) => acc + (Number(v.precio_venta_unitario || 0) * Number(v.cantidad || 0)), 0);
        const totalInyeccionesCapital = fL.filter(f => f && ['Ingreso Adicional','Inversión Inicial'].includes(f.tipo)).reduce((acc, f) => acc + (Number(f.monto) || 0), 0);
        const totalEgresosGlobales = fL.filter(f => f && ['Gasto Local','Inversión (Mercadería)','Retiro Personal'].includes(f.tipo)).reduce((acc, f) => acc + (Number(f.monto) || 0), 0);

        // PUNTO DE EQUILIBRIO
        const mesStr = hoyStr.substring(0,7);
        const metaEgresosMes = fL.filter(f => f && getFechaPeru(f.created_at).substring(0,7) === mesStr && ['Gasto Local','Retiro Personal'].includes(f.tipo)).reduce((acc, f) => acc + (Number(f.monto) || 0), 0);
        const utilidadMesReal = vL.filter(v => v && getFechaPeru(v.created_at).substring(0,7) === mesStr && v.estado_pedido !== 'Pendiente de Pago').reduce((a,b) => a + (Number(b.ganancia_total) || 0), 0);
        ...
        return { 
            ...
            cajaGlobal: (totalEntradasVentas + totalInyeccionesCapital - totalEgresosGlobales),
            disponibleBoveda: (totalGananciaBrutaVentas - totalGastosDeGanancias),
            pe_progreso: metaEgresosMes > 0 ? (utilidadMesReal / metaEgresosMes) * 100 : (utilidadMesReal > 0 ? 100 : 0),
            pe_ganActual: utilidadMesReal,
            pe_egMeta: metaEgresosMes
        };
    } catch (e) { 
        console.error("Falla en Cálculos:", e);
        return { cajaHoy: 0, ganHoy: 0, cajaGlobal: 0, disponibleBoveda: 0, pe_progreso:0, pe_ganActual:0, pe_egMeta:0 }; 
    }
  }, [finanzas, ventas]);

  // 4. ANALÍTICA DE PRODUCTOS (SEGURA)
  const analyticsMaster = useMemo(() => {
    try {
        const rankingProf = {};
        (ventas || []).forEach(v => {
            if (!v) return;
            const pFound = productos.find(prod => prod.id === v.producto_id);
            const name = pFound ? pFound.nombre : "Modelo Borrado";
        ...
        const dormido = (productos || []).filter(p => {
            if (!p) return false;
            const ultVenta = (ventas || []).filter(v => v && v.producto_id === p.id).pop();
            if (!ultVenta) return true;
            const diffDias = Math.floor((fechaHoy - new Date(ultVenta.created_at)) / (1000 * 60 * 60 * 24));
  ...
  const dataChartROI = [
    { n: 'Capital de Almacén', v: valorizacionInventario.cost || 0, fill: '#1E1B1C' },
    { n: 'Venta Proyectada', v: valorizacionInventario.vent || 0, fill: FUCSIA_PRINCIPAL }
  ];
  ...
  const handleWhatsAppDirecto = (grupo) => {
    let t = `¡Hola *${grupo.cliente_nombre}*! 👋 Ticket de BJ Importaciones.%0A%0A`;
    grupo.items.forEach(v => {
        const pMatch = productos.find(p => p.id === v.producto_id);
        t += `- *${v.cantidad}x* ${pMatch?.nombre || 'Producto'} (${v.color}): S/ ${(v.precio_venta_unitario * v.cantidad).toFixed(2)}%0A`;
    });
  ...
  const handleUpdateLibroManual = async () => {
    const ml = handleInputMonto(String(formEditFinanza.monto));
    await supabase.from('finanzas').update({ 
        tipo: formEditFinanza.tipo, descripcion: formEditFinanza.descripcion, monto: Number(ml), origen: formEditFinanza.origen 
    }).eq('id', idFinanzaEditando);
    setIdFinanzaEditando(null); alert("✅ Actualizado.");
  };

  const handleExportarExcelFinal = () => {
  ...
  if (cargando) return <div style={{ height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', backgroundColor:'#FFF5F7', color:FUCSIA_PRINCIPAL, fontWeight:'900', fontSize:'1.5rem' }}>CARGANDO BUNKER BJ v46... 🚀💎</div>;
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#FFF5F7', color: '#1E1B1C', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <header style={{ backgroundColor: '#ffffff', padding: '20px 5%', position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: `0 4px 15px rgba(0,0,0,0.06)`, flexWrap: 'wrap', gap: '15px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div><h1 style={{ margin: 0, fontSize: '1.3rem', fontWeight: '900', color: FUCSIA_PRINCIPAL }}>BJ IMPORTACIONES</h1><small style={{ color: '#64748B', fontWeight: '900', fontSize: '10px' }}>CHICLAYO • v46 MAESTRO INMORTAL</small></div>
        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '25px' }}>
              <div style={sCrd}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={handleExportarExcelFinal} style={{ background:`${FUCSIA_PRINCIPAL}20`, border:'none', padding:'8px 15px', borderRadius:'10px', color:FUCSIA_PRINCIPAL, fontWeight:'900', fontSize:'10px' }}>EXCEL</button>
                </div>
                  ...
                  return (
                    <div key={p.id} style={{ border: '1px solid #FFF1F2', padding: '22px', borderRadius: '35px', backgroundColor: '#fff', position: 'relative', boxShadow: '0 10px 30px rgba(0,0,0,0.04)' }}>
                      ...
                      <div style={{ margin: '10px 0', padding: '10px', backgroundColor: p.stock < 5 ? '#FFF1F2' : '#F0FDF4', borderRadius: '18px', textAlign: 'center' }}>
                        <span style={{ fontSize: '13px', fontWeight: '900', color: p.stock < 5 ? ROJO_ALERTA : VERDE_BJ }}>STOCK: {p.stock}</span>
                      </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '20px' }}>
                    {logisticaDividida.cuentasPorCobrar.map((grupo, idx) => (
                        <div key={idx} style={{ padding: '25px', backgroundColor: '#FFFBEB', borderRadius: '25px', border:`1px solid ${AMARILLO_BJ}40` }}>
                            ...
                            <button onClick={() => cobrarVentaPendiente(grupo)} style={{ width: '100%', backgroundColor: VERDE_BJ, color: '#fff', border: 'none', padding: '18px', borderRadius: '15px', fontWeight: '900', cursor:'pointer' }}>💰 REGISTRAR PAGO (COBRAR)</button>
                        </div>
                    ))}
                </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '45px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '30px' }}>
                ...
                <div style={sCrd}>
                    ...
                    <div style={{display:'flex', justifyContent:'space-between', fontSize:'13px'}}>
                        <span>Utilidad Mes: S/ {balanceFinancieroBunker.pe_ganAct.toFixed(2)}</span>
                        <strong>Meta Gasto: S/ {balanceFinancieroBunker.pe_egMeta.toFixed(2)}</strong>
                    </div>
                    {balanceFinancieroBunker.pe_progreso < 100 ? <small style={{color:ROJO_BJ, fontWeight:'bold', display:'block', marginTop:'5px'}}>Faltan S/ {(balanceFinancieroBunker.pe_egMeta - balanceFinancieroBunker.pe_ganAct).toFixed(2)} de utilidad para cubrir gastos operativos.</small> : <small style={{color:VERDE_BJ, fontWeight:'bold', display:'block', marginTop:'5px'}}>¡Meta superada! Tu negocio es rentable este mes.</small>}
                </div>

                {/* BÓVEDA DISPONIBLE */}
                <div style={{ ...sCrd, backgroundColor: '#1E1B1C', color: '#fff', border:`4px solid ${FUCSIA_PRINCIPAL}` }}>
                    <h4 style={{margin:0, color:FUCSIA_PRINCIPAL, fontSize:'14px', fontWeight:'900', marginBottom:'15px'}}>💰 DISPONIBLE PARA RETIRO (UTILIDAD NETA ACUMULADA)</h4>
                    <h3 style={{fontSize:'3.2rem', margin:0}}>S/ {balanceFinancieroBunker.disponibleBoveda.toFixed(2)}</h3>
                    <small style={{opacity:0.6}}>Ganancia total histórica restando los retiros realizados desde la bolsa de "Ganancias".</small>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(480px, 1fr))', gap: '45px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
                <div style={sCrd}>
                  <h4 style={{ marginTop: 0, marginBottom: '25px', fontWeight: '900', fontSize:'1.2rem' }}>🏆 PRODUCTOS MÁS RENTABLES (UTILIDAD)</h4>
            ...
            <div style={sCrd}>
                ...
                <div style={{ height: '350px', width: '100%' }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={dataChartROI} margin={{ top: 25, right: 30, left: -5, bottom: 20 }}>
                            ...
                            <Bar dataKey="v" radius={[20, 20, 0, 0]} barSize={80}>
                                {dataChartROI.map((entry, index) => ( <Cell key={`cell-${index}`} fill={entry.fill} /> ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                </div>
