...
export default function SistemaBJCMasterFinal() {
  ...
  // EDICIÓN EN GESTIÓN (FINANZAS Y PRODUCTOS)
  ...
  // ============================================================
  // [3] COMUNICACIÓN CON SUPABASE (TIEMPO REAL)
  // ============================================================
  ...
  useEffect(() => {
    ...
    const canalVentas = supabase.channel('master-v34-ventas')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'ventas' }, () => cargarTodo())
      .subscribe();
    const canalProductos = supabase.channel('master-v34-prod')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'productos' }, () => cargarTodo())
      .subscribe();
    const canalFinanzas = supabase.channel('master-v34-fin')
  ...
  // ============================================================
  // [4] CALCULOS BLINDADOS (USEMEMO)
  // ============================================================

  // --- LOGÍSTICA: BUSCA PENDIENTES DE CUALQUIER FECHA ---
  ...
  // --- HISTORIAL: AGRUPA VENTAS DEL DÍA POR CLIENTE ---
  ...
  const balanceContable = useMemo(() => {
    ...
    const gananciaNetaTotal = listV.reduce((acc, v) => acc + Number(v.ganancia_total ?? 0), 0);
    ...
    return { 
        ...
        gananciaTotalReal: gananciaNetaTotal
    };
  }, [finanzas, ventas]);

  // --- VALORIZACIÓN DE STOCK (INVERSIÓN) ---
  const auditoriaInventario = useMemo(() => {
    let costo = 0; let venta = 0; let unidades = 0;
    (productos || []).forEach(p => { 
        const st = Number(p.stock ?? 0);
        if (st > 0) { 
            costo += (Number(p.precio_compra ?? 0) * st); 
            venta += (Number(p.precio_venta ?? 0) * st); 
            unidades += st; 
        } 
    });
    return { costo, venta, unidades, utilidad: venta - costo };
  }, [productos]);

  const dataROI = [
    { name: 'Inversión Real', val: auditoriaInventario.costo || 0, fill: '#1E1B1C' },
    { name: 'Retorno Potencial', val: auditoriaInventario.venta || 0, fill: FUCSIA_PRINCIPAL }
  ];
  ...
  const handleClienteChange = (e) => {
    ...
    const found = ventas.find(vent => vent.cliente_nombre?.toLowerCase() === v.toLowerCase());
    if (found) { setLocalidad(found.localidad || ''); setTelefono(found.telefono || ''); }
  };
  ...
  const addCarrito = (p) => {
    ...
    if (Number(p.stock ?? 0) < c + yaEn) return alert("No hay suficiente stock físico.");
  ...
  const enviarWhatsAppTicket = (grupo) => {
    let msg = `¡Hola *${grupo.cliente_nombre}*! 👋 Resumen de tu compra en *B J Importaciones*.%0A%0A`;
  ...
  const finalizarVenta = async (est = 'Entregado') => {
  ...
  const corregirCantidadHistorial = async (v) => {
    ...
    if ((pRef?.stock ?? 0) < diff) return alert("Error: No hay suficiente stock disponible.");
  ...
  const anularItemHistorial = async (v) => {
    if (confirm("¿Anular este producto? El stock se devolverá.")) {
  ...
  const registrarGastoManual = async (e) => {
    ...
    const { error } = await supabase.from('finanzas').insert([{...formFinanzas, monto: Number(cleanM)}]);
    if(!error) { setFormFinanzas({tipo:'Gasto Local', descripcion:'', monto:''}); alert("✅ Registro guardado."); }
  };
  ...
  const addNuevoProducto = async (e) => {
    ...
    if (!error) { setFormProd({ nombre: '', precio_compra: '', precio_venta: '', precio_menor: '', stock: '', colores: '' }); alert("✨ Modelo cargado."); }
  };
  ...
  const s_input = { padding: '16px', borderRadius: '16px', border: `2px solid #FCC2E2`, width: '100%', outline: 'none', fontSize: '15px', boxSizing: 'border-box', backgroundColor: '#fff', transition: '0.3s' };
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#FFF5F7', color: '#1E1B1C', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <header style={{ backgroundColor: '#ffffff', padding: '20px 5%', position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: `0 4px 15px rgba(0,0,0,0.06)`, flexWrap: 'wrap', gap: '15px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          <div style={{ backgroundColor: FUCSIA_PRINCIPAL, color: '#fff', width: '50px', height: '50px', borderRadius: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '900', fontSize: '1.5rem', boxShadow: `0 5px 15px ${FUCSIA_PRINCIPAL}40` }}>BJ</div>
          <div>
            <h1 style={{ margin: 0, fontSize: '1.4rem', fontWeight: '900', color: FUCSIA_PRINCIPAL, letterSpacing: '-0.8px' }}>B J IMPORTACIONES</h1>
            <small style={{ color: '#64748B', fontWeight: '900', fontSize: '10px', textTransform:'uppercase' }}>CHICLAYO • MAESTRO v34</small>
          </div>
        </div>
      ...
      <main style={{ maxWidth: '1400px', margin: '0 auto', padding: '35px 20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '35px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '25px' }}>
              <div style={s_card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button onClick={exportarExcelCaja} style={{ backgroundColor: `#FCA5D430`, border: 'none', padding: '10px 18px', borderRadius: '12px', color: FUCSIA_PRINCIPAL, cursor: 'pointer', fontWeight: '900', fontSize: '11px' }}>REPALDO EXCEL</button>
                </div>
                <h2 style={{ margin: '20px 0', fontSize: '3.5rem', fontWeight: '900' }}>S/ {balanceContable.cajaHoy.toFixed(2)}</h2>
                <div style={{ color: '#16A34A', fontWeight: '900', fontSize: '16px', backgroundColor: '#F0FDF4', padding: '10px 20px', borderRadius: '15px', display: 'inline-block' }}>Ganancia Día: S/ {balanceContable.gananciaHoy.toFixed(2)}</div>
              </div>
              <div style={s_card}>
            ...
            <div style={{ ...s_card, border: `3px solid #FCA5D4` }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '35px' }}>
                <input list="clients" placeholder="👤 Nombre del Cliente" value={cliente} onChange={handleClienteChange} style={s_input} />
                ...
                <input placeholder="📍 Zona / Pueblo / Distrito" value={localidad} onChange={e => setLocalidad(e.target.value)} style={s_input} />
              </div>
                  ...
                  {carrito.map((item, idx) => (
                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #FCC2E2', paddingBottom: '18px', marginBottom: '12px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                        ...
                        <button onClick={() => { const n = [...carrito]; n.splice(idx, 1); setCarrito(n); }} style={{ color: '#fff', backgroundColor: FUCSIA_PRINCIPAL, border: 'none', borderRadius: '50%', width: '45px', height: '45px', cursor: 'pointer' }}>X</button>
                      </div>
                    </div>
                  ))}
                  <div style={{ textAlign: 'right', marginTop: '30px' }}>
                    <span style={{ fontSize: '14px', fontWeight: '900' }}>DESCUENTO GLOBAL: S/ </span>
                    ...
                    <h3 style={{ margin: '15px 0', fontSize: '2.8rem', fontWeight: '900' }}>Total: S/ {(carrito.reduce((acc, i) => acc + (Number(i.precio_venta) * i.cantidad), 0) - Number(descuento)).toFixed(2)}</h3>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '25px', marginTop: '25px' }}>
                    <button onClick={() => finalizarVenta('Entregado')} style={{ backgroundColor: '#1E1B1C', color: '#fff', border: 'none', padding: '25px', borderRadius: '22px', fontWeight: '900', cursor: 'pointer', fontSize: '17px' }}>✅ PAGAR Y ENTREGAR</button>
                    <button onClick={() => finalizarVenta('En Almacén')} style={{ backgroundColor: FUCSIA_PRINCIPAL, color: '#fff', border: 'none', padding: '25px', borderRadius: '22px', fontWeight: '900', cursor: 'pointer', fontSize: '17px' }}>📦 PAGAR Y ALMACENAR</button>
                  </div>
                </div>
              )}

              <input placeholder="🔍 Buscar modelo en el catálogo..." value={busqueda} onChange={e => setBusqueda(e.target.value)} style={{ ...bjInput, marginBottom: '25px', height: '65px', fontSize: '19px' }} />
                  ...
                  return (
                    <div key={p.id} style={{ border: '1px solid #FFF1F2', padding: '25px', borderRadius: '35px', backgroundColor: '#fff', position: 'relative', boxShadow: '0 10px 30px rgba(0,0,0,0.04)' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginBottom: '22px', alignItems: 'center' }}>
                        <button onClick={() => setCantidades({...cantidades, [p.id]: Math.max(1, (cantidades[p.id] || 1) - 1)})} style={{ border: 'none', background: '#f5f5f5', borderRadius: '12px', width: '50px', height: '50px', cursor:'pointer' }}>-</button>
                        <span style={{ fontWeight: '900', fontSize: '24px' }}>{cantidades[p.id] || 1}</span>
                        <button onClick={() => setCantidades({...cantidades, [p.id]: Math.min(p.stock, (cantidades[p.id] || 1) + 1)})} style={{ border: 'none', background: '#f5f5f5', borderRadius: '12px', width: '50px', height: '50px', cursor:'pointer' }}>+</button>
                      </div>
                    ...
                    {editandoGrupoId === grupo.id_grupo ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '25px', backgroundColor:'#fff', padding:'25px', borderRadius:'25px', border: `2px solid ${FUCSIA_PRINCIPAL}` }}>
                         ...
                         <div style={{ display:'flex', gap:'15px', marginTop:'10px' }}>
                            <button onClick={() => guardarCambiosCliente(grupo)} style={{ backgroundColor:'#16A34A', color:'#fff', padding:'20px', borderRadius:'18px', flex:2, fontWeight:'900' }}>GUARDAR</button>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        ...
                        <div key={v.id} style={{ backgroundColor: '#fff', padding: '22px', borderRadius: '25px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ display:'flex', gap:'25px', alignItems:'center' }}>
                                ...
                                <button onClick={() => anularItemHistorial(v)} style={{ border:'none', background:'none', color: FUCSIA_PRINCIPAL, cursor:'pointer', fontSize: '26px' }}>🗑️</button>
                            </div>
                        </div>
                        ))}
                    </div>
                    ...
                    <div key={idx} style={{ ...bjCard, borderLeft: `15px solid ${FUCSIA_PRINCIPAL}` }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '30px' }}>
                            <div>
                                ...
                                <p style={{ fontSize: '16px', color: '#64748B', fontWeight:'900', margin: '5px 0' }}>📍 ZONA: {grupo.localidad}</p>
                            </div>
                            <button onClick={() => {
        ...
        {/* ===================== [TAB GESTIÓN - BLINDADA] ===================== */}
        {vista === 'contabilidad' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '45px' }}>
            
            <div style={{ ...bjCard, border: `4px solid ${FUCSIA_PRINCIPAL}` }}>
                <h3 style={{ margin: 0, color: FUCSIA_PRINCIPAL, marginBottom: '35px', fontWeight: '900', fontSize:'2rem' }}>📊 Salud del Negocio</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '35px' }}>
                    <div style={{ backgroundColor: '#F8FAFC', padding: '30px', borderRadius: '35px' }}>
                        <small style={{ fontWeight: '900', color: '#64748B', textTransform: 'uppercase' }}>CAPITAL TOTAL EN STOCK (COSTO)</small>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '25px' }}>
              <div style={{ ...bjCard, borderLeft: `12px solid ${FUCSIA_PRINCIPAL}`, padding: '25px' }}><small style={{fontWeight:'bold', opacity:0.6}}>EGRESOS</small><h4 style={{fontSize:'1.8rem', margin:'10px 0'}}>S/ {Number(resumenFinanciero.egresosTotales ?? 0).toFixed(2)}</h4></div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(500px, 1fr))', gap: '45px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '45px' }}>
                <div style={bjCard}>
                  ...
                  <div style={{ maxHeight: '480px', overflowY: 'auto' }}>
                    {productos.filter(p => p.nombre.toLowerCase().includes(busquedaStock.toLowerCase())).map(p => {
                        ...
                        return (
                          <div key={p.id} style={{ padding: '22px', border: '1px solid #f1f1f1', borderRadius: '30px', backgroundColor: '#fff', marginBottom: '18px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
                              ...
                              <button onClick={async () => { if(confirm("¿Borrar?")) await supabase.from('productos').delete().eq('id', p.id); }} style={{ background: 'none', border: 'none', color: '#CBD5E1', cursor:'pointer', fontSize: '1.5rem' }}>🗑️</button>
                            </div>
                ...
                <div style={bjCard}>
                  ...
                  <h4 style={{ marginTop: 0, marginBottom: '30px', fontWeight: '900', fontSize: '1.2rem' }}>📖 Libro Diario Detallado</h4>
                            ...
                            {idFinanzaEditando === f.id ? (
                               <td colSpan="3" style={{ padding: '25px', backgroundColor: '#FFF5F7', borderRadius: '30px', border: `3px solid ${FUCSIA_PRINCIPAL}` }}>
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                      ...
                                      <div style={{display:'flex', gap:'15px', marginTop: '10px'}}><button onClick={updateGastoLibro} style={{backgroundColor:'#16A34A', color:'#fff', padding:'20px', borderRadius:'18px', border:'none', flex:2, fontWeight:'900' }}>OK</button><button onClick={()=>setIdFinanzaEditando(null)} style={{background:'#64748B', color:'#fff', padding:'20px', borderRadius:'18px', border:'none', flex:1}}>X</button></div>
                                  </div>
                               </td>
                            ) : (
                              <>
                                ...
                                <td style={{ textAlign: 'right', padding: '22px 10px' }}>
                                  ...
                                  <button onClick={async ()=> { if(confirm("Borrar registro?")) await supabase.from('finanzas').delete().eq('id', f.id); }} style={{background:'none', border:'none', color: FUCSIA_PRINCIPAL, cursor:'pointer', fontSize: '1.4rem'}}>🗑️</button>
                                </td>
                              </>
                            )}
                          </tr>
